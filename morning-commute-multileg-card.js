...
class MorningCommuteMultilegCard extends HTMLElement {
  ...
  _trainRow(train) {
    ...
    return `<div class="train-row" style="background:${hexToRgba(_bg, 0.12)};border-left:3px solid ${hexToRgba(_bg, 0.4)}"><div class="t-top"><span class="t-time">${dep}</span><div class="t-meta">${platHtml}${jHtml}</div><span class="t-status" style="color:${status.color}">${status.icon} ${delay>0?`+${delay}m`:status.label}</span></div>${opHtml}${callHtml}${delayHtml}${cancelHtml}</div>`;
  }
